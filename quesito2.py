...
# 3. DATASET & SPLIT
# ─────────────────────────────────────────
# Assegniamo la trasformazione corretta a ciascun ImageFolder
...
class ShallowCNN(nn.Module):
    def __init__(self, num_classes: int = 15):
        ...
        self.fc      = nn.Linear(32 * 16 * 16, num_classes)   # ← 32 non 16
...
# 1. Crea il modello e spostalo sulla GPU/CPU
model = ShallowCNN(num_classes=NUM_CLASSES).to(device)

# 2. Applica la funzione di inizializzazione ai pesi e ai bias
...
# 5. LOSS & OPTIMIZER
# ─────────────────────────────────────────
criterion = nn.CrossEntropyLoss()   # layer 12: crossentropyex
...
if __name__ == "__main__":
    ...
    for epoch in range(1, NUM_EPOCHS + 1):
        ...
        if vl_acc > best_val_acc:
            best_val_acc = vl_acc
            torch.save(model.state_dict(), "CVPR2023_project_2_and_3_data/best_model_quesito2.pth")
    ...
    plt.tight_layout()
    plt.savefig("CVPR2023_project_2_and_3_data/training_curvesQuesito2.png", dpi=150)
    plt.show()
    ...
    model.load_state_dict(torch.load("CVPR2023_project_2_and_3_data/best_model_quesito2.pth", map_location=device))
    model.eval()
    ...
    plt.tight_layout()
    plt.savefig("CVPR2023_project_2_and_3_data/confusion_matrix_testsetQuesito2.png", dpi=150)
    plt.show()
